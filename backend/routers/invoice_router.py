...
from services.dawn_service import apply_pastel_tone_for_duplicates
...
def find_best_match(order_row, candidates_df, original_df, threshold=0.8, invoice_prod_col=None, order_prod_col=None, code_col=None, code_filter=None):
    ...
    # 1. 집하지점코드 등 필터 적용
    candidates = candidates_df[candidates_df['matched'] == False]
    ...
    if not candidates.empty:
        ...
        if not matches.empty:
            ...
            best_idx = matches['name_similarity'].idxmax()  # 이 인덱스는 original_df의 인덱스임
            best_match = original_df.loc[best_idx]  # 반드시 원본에서 가져오기
            original_df.at[best_idx, 'matched'] = True  # 원본에 True로!
...
async def invoice_match(
    ...
    """일반 송장매칭 - GUI와 완전 동일한 3파일 처리"""
    try:
        logger.info("일반 송장매칭 시작 (3파일 처리)")
        
        # 임시 디렉토리 생성
        with tempfile.TemporaryDirectory() as temp_dir:
            # 분류완료 주문파일 저장
            order_path = os.path.join(temp_dir, classified_file.filename)
            ...
            day_invoice_df = pd.read_excel(day_invoice_path)
            dawn_invoice_df = pd.read_excel(dawn_invoice_path) if dawn_invoice_path else None
            
            logger.info(f"주문 데이터: {len(order_df)}행, 당일송장: {len(day_invoice_df)}행, "
                       f"새벽송장: {len(dawn_invoice_df) if dawn_invoice_df is not None else 0}행")
            
            # 주문 데이터 컬럼 매핑 - GUI와 완전 동일
            ...
            # 당일배송 송장 컬럼 매핑
            day_name_col = auto_map_col(day_invoice_df, ['받는분', '수령인', '수취인'])
            day_addr_col = auto_map_col(day_invoice_df, ['받는분주소', '수령인주소', '주소'], force_addr=True)
            day_code_col = auto_map_col(day_invoice_df, ['배송점', '집하지점코드'])
            day_invoice_col = auto_map_col(day_invoice_df, ['송장번호', '운송장번호'])
            day_prod_col = auto_map_col(day_invoice_df, ['상품주문번호', '주문번호', '거래처주문코드', '거래처 주문번호', '거래처주문번호'])
            
            # 새벽배송 송장 컬럼 매핑 (있는 경우)
            dawn_name_col = dawn_addr_col = dawn_invoice_col = dawn_prod_col = None
            if dawn_invoice_df is not None:
                dawn_name_col = auto_map_col(dawn_invoice_df, ['받는분', '수령인', '수취인', '수령자'])
                dawn_addr_col = auto_map_col(dawn_invoice_df, ['받는분주소', '수령인주소', '주소', '수령자 주소'], force_addr=True)
                dawn_invoice_col = auto_map_col(dawn_invoice_df, ['운송장번호', '송장번호'])
                dawn_prod_col = auto_map_col(dawn_invoice_df, ['상품주문번호', '주문번호', '거래처주문코드', '거래처 주문번호', '거래처주문번호'])
            
            # 필수 컬럼 확인
            if not all([order_name_col, order_addr_col, order_code_col, day_name_col, day_addr_col, day_code_col, day_invoice_col]):
                missing_cols = []
                if not order_name_col: missing_cols.append("주문파일-받는분")
                if not order_addr_col: missing_cols.append("주문파일-주소")  
                if not order_code_col: missing_cols.append("주문파일-집하지점코드")
                if not day_name_col: missing_cols.append("당일송장파일-받는분")
                if not day_addr_col: missing_cols.append("당일송장파일-주소")
                if not day_code_col: missing_cols.append("당일송장파일-집하지점코드")
                if not day_invoice_col: missing_cols.append("당일송장파일-송장번호")
            ...
            day_invoice_df['__clean_name'] = day_invoice_df[day_name_col].apply(clean_address_for_matching)
            day_invoice_df['__clean_addr'] = day_invoice_df[day_addr_col].apply(clean_address_for_matching)
            day_invoice_df['matched'] = False
            
            if dawn_invoice_df is not None:
                dawn_invoice_df['__clean_name'] = dawn_invoice_df[dawn_name_col].apply(clean_address_for_matching)
                dawn_invoice_df['__clean_addr'] = dawn_invoice_df[dawn_addr_col].apply(clean_address_for_matching)
                dawn_invoice_df['matched'] = False
            ...
            for idx, row in order_df.iterrows():
                ...
                if code == '' or code == 'nan':
                    ...
                    candidates = day_invoice_df[day_invoice_df['matched'] == False]
                    candidates = candidates[
                        ~candidates[day_code_col].astype(str).str.startswith('105') &
                        ~candidates[day_code_col].astype(str).str.startswith('108')
                    ]
                    
                    best_match, similarity_scores = find_best_match(
                        row, candidates, day_invoice_df, threshold=0.8,
                    ...
                    if best_match is not None:
                        ...
                        # matched 플래그는 find_best_match에서 이미 설정됨
                        matched = True
                        
                elif code_str == '105':
                    # 택배 매칭
                    candidates = day_invoice_df[
                        (day_invoice_df['matched'] == False) &
                        (day_invoice_df[day_code_col].astype(str).str.startswith('105'))
                    ]
                ...
                # 새벽 매칭
                if not matched and code_str == '108' and dawn_invoice_df is not None:
                    candidates = dawn_invoice_df[dawn_invoice_df['matched'] == False]
                    
                    best_match, similarity_scores = find_best_match(
                        row, candidates, dawn_invoice_df, threshold=0.8,
                        invoice_prod_col=dawn_prod_col,
                        order_prod_col=order_prod_col
                    )
                ...
                if matched:
                    matched_count += 1
                    log_matching_details(row, best_match, similarity_scores, True)
                else:
            ...
            # 매칭 결과 분석
            analysis = analyze_matching_results(order_df, day_invoice_df, unmatched_rows)
            ...
            # 중복주소 파스텔톤 적용
            apply_pastel_tone_for_duplicates(matched_path)
            if not unmatched_df.empty:
                apply_pastel_tone_for_duplicates(unmatched_path)
            ...
            return {
                ...
                "analysis": analysis,
                "matched_file": f"/api/invoice-match/files/download/{matched_filename}",
                "unmatched_file": f"/api/invoice-match/files/download/{unmatched_filename}" if not unmatched_df.empty else None
            }
...
async def dawn_invoice_match(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            invoice_prod_col = auto_map_col(invoice_df, ['상품주문번호', '주문번호', '거래처주문코드', '거래처 주문번호', '거래처주문번호'])
            ...
            for idx, row in order_df.iterrows():
                ...
                if order_prod_col and invoice_prod_col and pd.notna(prod_code):
                    match_df = invoice_df[
                        (invoice_df[invoice_prod_col] == prod_code) & 
                        (invoice_df['matched'] == False)
                    ]
                ...
                # 매칭 결과 적용 - GUI와 동일하게 최종 확정 시 matched 설정
...
@router.post("/general-invoice-match/")
async def general_invoice_match(
    order_file: UploadFile = File(...),
    invoice_file: UploadFile = File(...)
):
    """일반 송장매칭 - GUI의 start_invoice_matching 완전 동일"""
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # 파일 저장
            order_path = os.path.join(temp_dir, order_file.filename)
            invoice_path = os.path.join(temp_dir, invoice_file.filename)
            
            with open(order_path, "wb") as f:
                shutil.copyfileobj(order_file.file, f)
            with open(invoice_path, "wb") as f:
            ...
            order_addr_col = auto_map_col(order_df, ['받는분주소', '수령인주소', '주소', '수령자 도로명 주소', '수령자 주소'], force_addr=True)
            order_prod_col = auto_map_col(order_df, ['상품주문번호', '주문번호', '거래처주문코드'])
            order_code_col = auto_map_col(order_df, ['집하지점코드', '지점코드'])
            ...
            # 집하지점코드별 매칭 - GUI와 완전 동일한 로직
            for idx, row in order_df.iterrows():
                pickup_code = str(row.get(order_code_col, '')) if order_code_col else ''
                
                # 배송사 결정
                delivery_company = ''
                code_filter = None
                
                if pickup_code.startswith('105'):
                    delivery_company = '롯데택배'
                    code_filter = '105'
                elif pickup_code.startswith('108'):
                    delivery_company = '컬리넥스트마일'
                    code_filter = '108'
                else:
                    delivery_company = '우리한방택배'
                    code_filter = ''
                
                logger.debug(f"집하지점코드: {pickup_code} -> 배송사: {delivery_company}")
                
                # 매칭 실행
                match, similarity_scores = find_best_match(
                    row, 
                    invoice_df, 
                    order_df,
                    threshold=0.8,
                    invoice_prod_col=invoice_prod_col,
                    order_prod_col=order_prod_col,
                    code_col=None,  # 집하지점코드 필터링은 여기서는 사용하지 않음
                    code_filter=None
                )
                ...
                if match is not None:
                    order_df.at[idx, '송장번호'] = str(match[invoice_num_col])
                    order_df.at[idx, '배송사명'] = delivery_company
                    matched_count += 1
                    log_matching_details(row, match, similarity_scores, True)
                else:
                    order_df.at[idx, '배송사명'] = delivery_company  # 미매칭이어도 배송사명은 설정
                    unmatched_rows.append(row)
                    log_matching_details(row, None, None, False)
            ...
            # 매칭 결과 분석
            analysis = analyze_matching_results(order_df, invoice_df, unmatched_rows)
...
async def download_file(filename: str):
    """결과 파일 다운로드"""
    file_path = os.path.join('data', 'user', filename)
